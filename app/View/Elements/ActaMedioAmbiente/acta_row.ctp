...
<table class="table" id="table_content_actas">
    ...
    <tbody>
        ...
        ?>
        <tr class="acta_med_amb_row_container" acta_med_amb_id="<?php echo $acta->getAttr('id'); ?>" style="<?php echo ($acta->getAttr('revisado')==0)?'background-color:#BADEFB' : ''; ?>">
            ...
            <center>
                ...
                <?php if(($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id')) || $this->Session->read('Auth.User.tipo_user_id') == 1 || $this->Session->read('Auth.User.tipo_user_id') == 3) { ?>
                    <a href="<?= ENV_WEBROOT_FULL_URL; ?>acta_medio_ambientes/editar_informe_med_amb/<?php echo $acta->getAttr('id')?>"><i class="fa fa-pencil fa-lg"></i> </a>| 
                <?php } ?>
                ...
                <?php if((($this->Session->read('Auth.User.tipo_user_id') == 2) && ($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id'))) || $this->Session->read('Auth.User.tipo_user_id') == 1) { ?>
                    |
                    <a href="#myModalDeleteActaMedioAmbiente" role="button" data-toggle="modal" class="<?php if(($this->Session->read('Auth.User.tipo_user_id') == 2) && ($this->Session->read('Auth.User.id') != $acta->getAttr('reponsable_sup_id'))) { ?>not-active<?php }?>"><i class="fa fa-times open-model-delete-acta-med-amb fa-lg"></i> </a>
                <?php } ?>  
