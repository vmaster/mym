...
<table class="table" id="table_content_actas">
    ...
    <tbody>
        ...
        ?>
        <tr class="acta_instalacion_row_container" acta_instalacion_id="<?php echo $acta->getAttr('id'); ?>" style="<?php echo ($acta->getAttr('revisado')==0)?'background-color:#BADEFB' : ''; ?>">
            ...
            <center>
                ...
                <?php if(($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id')) || $this->Session->read('Auth.User.tipo_user_id') == 1 || $this->Session->read('Auth.User.tipo_user_id') == 3) { ?>
                    <a href="<?= ENV_WEBROOT_FULL_URL; ?>acta_instalaciones/editar_informe_instalacion/<?php echo $acta->getAttr('id')?>"><i class="fa fa-pencil fa-lg"></i> </a>| 
                <?php } ?>
                <a href="<?= ENV_WEBROOT_FULL_URL; ?>acta_instalaciones/view_informe/<?php echo $acta->getAttr('id')?>" target="_blank"><i class="fa fa-search fa-lg"></i> </a>
                <?php if((($this->Session->read('Auth.User.tipo_user_id') == 2) && ($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id'))) || $this->Session->read('Auth.User.tipo_user_id') == 1) { ?>
                    |
                    <a href="#myModalDeleteActaInstalacion" role="button" data-toggle="modal" class="<?php if(($this->Session->read('Auth.User.tipo_user_id') == 2) && ($this->Session->read('Auth.User.id') != $acta->getAttr('reponsable_sup_id'))) { ?>not-active<?php }?>"><i class="fa fa-times open-model-delete-acta-instal fa-lg"></i> </a>
                <?php } ?>  
