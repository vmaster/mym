...
<table class="table" id="table_content_actas">
	...
	<tbody>
		...
		<tr class="acta_row_container" acta_id="<?php echo $acta->getAttr('id'); ?>" style="<?php echo ($acta->getAttr('revisado')==0)?'background-color:#BADEFB' : ''; ?>">
			...
			<td>
			<center>
				<?php if(($this->Session->read('Auth.User.tipo_user_id') != 3) && ($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id'))) { ?>
				...
				<?php if(($this->Session->read('Auth.User.tipo_user_id') == 2) && ($this->Session->read('Auth.User.id') == $acta->getAttr('reponsable_sup_id'))) { ?>
					|
